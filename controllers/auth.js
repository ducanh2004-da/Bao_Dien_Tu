    ...
    try {
        ...
        User.findByEmail(email, async (err, user) => {
            if (!user) {
                ...
                return res.render('account/sendOtp', { error: 'Không tìm thấy email nào' });
            }
            ...
            try {
                ...
                res.render('account/checkOtp', { message: 'OTP đã được gửi đến email của bạn' });
            } catch (emailError) {
                console.log(emailError);
                res.status(500).render('account/sendOtp', { error: 'Có lỗi xảy ra khi gửi OTP.' });
            }
        });
    } catch (err) {
        console.log(err);
...
module.exports.checkOtp = (req,res) =>{
    ...
    res.redirect(`/api/reset-password?token=${crypto.randomBytes(20).toString('hex')}&email=${encodeURIComponent(email)}`)
}
...
module.exports.resetPass = async (req, res) => {
    ...
    try {
        ...
        User.resetPassword(email, hashedPassword, (err) => {
            if (err) {
                console.log(err);
                return res.render('account/resetPass', { error: 'Có lỗi xảy ra khi đặt lại mật khẩu.' });
            }
        ...
        console.error(error);
        res.render('account/resetPass', { error: 'Có lỗi xảy ra khi đặt lại mật khẩu.' });
    }
};
